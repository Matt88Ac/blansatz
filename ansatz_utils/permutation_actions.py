...
def permutation_sign(p: torch.Tensor) -> torch.Tensor:
    ...
    true_tensor = torch.ones(1)[0]
    for i in range(dim):
        if not visited[i]:
            count += 1
            k = i
            while not visited[k]:
                visited = visited.select_scatter(true_tensor, 0, k)
                k = p[k]
...
def vectorized_permutation_sign(p: torch.Tensor) -> torch.Tensor:
    ...
    signs = torch.stack([
        permutation_sign(p_i) for p_i in torch.unbind(p.contiguous().view(-1, p.size(-1)), dim=0)
    ], dim=0).view_as(p[..., 0])
    return signs
...
if __name__ == '__main__':
    ...
    # perms = perms.unsqueeze(1).repeat([1, 2, 1])
